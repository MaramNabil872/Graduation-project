...
const CartPage = () => {
    ...
    const updateCart = (updatedItems) => {
        ...
        const totalPrice = updatedItems.reduce((acc, item) => acc + (item.product.price * item.quantity), 0);
                    ...
                    {cart.items.map(item => (
                        <li key={item.product.id} className="cart-item">
                            <div className="item-details">
                                <Link to={`/products/${item.product.id}`} className="item-title">{item.product.product+name}</Link>
