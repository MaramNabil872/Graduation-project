...
const AddProduct = () => {
  ...
  const ProductForm = ({ product, handleChange, submitted }) => {
    const handleImageChange = (e) => {
        const file = e.target.files[0]; // Get the first file from the selected files
        handleChange({
            target: {
                name: 'imageUrl',
                value: URL.createObjectURL(file) // Create a URL for the selected file
            }
        });
    };
  const addProduct = () => {
      ...
      <div className="container">
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <Form.Group className="mb-3" controlId="productImageUrl">
                ...
                />
                {product.imageUrl && (
                    <img src={product.imageUrl} alt="Product Preview" style={{ maxWidth: '100%', marginTop: '10px' }} />
                )}
            </Form.Group>
