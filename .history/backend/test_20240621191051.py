...
def new_func(app):
    app.config['SQLALCHEMY_DATABASE_URI'] = 'ssql+pyodbc://DESKTOP-KMSRDT2\DELL:<>@DESKTOP-KMSRDT2\SQLEXPRESS/dishdash?driver=ODBC+Driver+17+for+SQL+Server'
    db = SQLAlchemy(app)
    return db
...
def register():
    ...
    # Validate input data
    if not data.get('first_name') or not data.get('last_name') or not data.get('email') or not data.get('password') or not data.get('phone_number') or not data.get('address'):
    ...
    if not send_email(to_mail, subject, body):
